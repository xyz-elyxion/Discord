...
func init() {
	if dir := os.Getenv("LIMEY_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using LIMEY_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../LimeyData")
		BaseDir = path.Join(dir, "..", "LimeyData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("LimeyV1").UserConfig()
	}
