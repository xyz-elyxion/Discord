...
func main() {
	...
	win = g.NewMasterWindow("Limey V1 Installer", 1200, 800, 0)
...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Failed to install the latest Limey V1 builds", "If this issue persists, visit https://limey-discord.onrender.com for help.", err.Error())
	}
	return
}
...
func handleErr(di *DiscordInstall, err error, action string) {
	...
	ShowModal("Failed to "+action+" this Install.", "If this issue persists, visit: https://limey-discord.onrender.com", err.Error())
}
	...
	layout := g.Layout{
		g.Style().SetFontSize(20).To(
			renderErrorCard(
				DiscordYellow,
				color.Black,
				"**limey-discord.onrender.com** is the only official place to get Limey V1. Any other site claiming to be us is malicious.\n"+
		...
		g.Style().SetFontSize(20).To(
			g.Row(
				g.Style().
					...
					SetDisabled(LatestHash == "Unknown").
					To(
						g.Button("Install").
							OnClick(handlePatch).
							Size((w-40)/4, 50),
		...
		InfoModal("#unpatched", "Uninstalled", "Limey V1 has been uninstalled!"),
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise Limey V1 will likely not work.\n\n"+
