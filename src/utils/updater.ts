...
export async function checkForUpdates() {
    changes = await Unwrap(LimeyV1Native.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(LimeyV1Native.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(LimeyV1Native.updater.rebuild()))
...
export const getRepo = () => Unwrap(LimeyV1Native.updater.getRepo());
