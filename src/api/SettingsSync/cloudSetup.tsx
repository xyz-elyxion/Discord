...
export async function checkCloudUrlCsp() {
    ...
    const res = await LimeyV1Native.csp.requestAddOverride(Settings.cloud.url, ["connect-src"], "Cloud Sync");
    if (res === "ok") {
        openModal(props => (
            <ConfirmModal
                {...props}
...
export async function getAuthorization() {
    const secrets = await DataStore.get<Record<string, string>>("LimeyV1_cloudSecret") ?? {};

    const origin = getCloudUrlOrigin();

    // we need to migrate from the old format here
    if (secrets[origin]) {
        await DataStore.update<Record<string, string>>("LimeyV1_cloudSecret", secrets => {
            secrets ??= {};
