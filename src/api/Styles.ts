...
export const styleMap = window.LimeyV1Styles ??= new Map();

export const limeyV1RootNode = document.createElement("limeyV1-root");
/**
 * Houses all Limey V1 core styles. This includes all imported css files
 */
export const coreStyleRootNode = document.createElement("limeyV1-styles");
/**
 * Houses all plugin specific managed styles
 */
export const managedStyleRootNode = document.createElement("limeyV1-managed-styles");
/**
 * Houses the user's themes and quick css
 */
export const userStyleRootNode = document.createElement("limeyV1-user-styles");

limeyV1RootNode.style.display = "none";
...
export function initStyles() {
    ...
    createAndAppendStyle("limeyV1-margins", coreStyleRootNode).textContent = generateMarginCss();

    LimeyV1Native.native.getRendererCss().then(css => rendererCssNode.textContent = css);
    if (IS_DEV) {
        LimeyV1Native.native.onRendererCssUpdate(newCss => {
...
document.addEventListener("DOMContentLoaded", () => {
    document.documentElement.append(limeyV1RootNode);
}, { once: true });
...
export function enableStyle(name: string) {
    ...
    if (!style.dom) {
        ...
        style.dom.dataset.limeyV1Name = style.name;
    }
    compileStyle(style);
