...
const AdditionalAllowedChannelIds = [
    "1024286218801926184", // Limey V1 > #bot-commands
];
...
async function generateDebugInfoMessage() {
    ...
    const commonIssues = {
        ...
        "Limey V1 DevBuild": !IS_STANDALONE,
...
export default definePlugin({
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Forms.FormText>You are using an outdated version of Limey V1! Chances are, your issue is already fixed.</Forms.FormText>
                                <Forms.FormText className={Margins.top8}>
            ...
            const roles = GuildMemberStore.getSelfMember(LIMEYV1_GUILD_ID)?.roles;
    ...
    renderMessageAccessory(props) {
        if (props.message.limeyV1EmbeddedBy) return null;
        ...
        if (props.channel.parent_id === SUPPORT_CATEGORY_ID && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/limeyV1-debug") || props.message.content.includes("/limeyV1-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
